...
const COOKIE_EXPIRATION_SECONDS = 60 * 10 // 10 mins

export class OAuthClient<T> {
    private readonly provider: oAuthProvidersType
    ...
    createAuthUrl(cookies: ResponseCookies) {
    ...
    private fetchToken(code: string, codeVerifier: string) {
        return fetch(this.urls.token, {
            ...
            .then((rawData) => {
                const { data, success, error } =
                    this.tokenSchema.safeParse(rawData)
...
export function getOAuthClient(provider: oAuthProvidersType) {
    switch (provider) {
        case 'google':
...
function createState(cookies: ResponseCookies) {
...
function createCodeVerifier(cookies: ResponseCookies) {
...
function validateState(state: string, cookies: ReadonlyRequestCookies) {
    const cookieState = cookies.get(STATE_COOKIE_KEY)?.value
    return cookieState === state
}

function getCodeVerifier(cookies: ReadonlyRequestCookies) {
